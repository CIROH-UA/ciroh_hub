  ...
  return (
    <div className={styles.hero_outer}>
      <svg viewBox="0 0 920 518" preserveAspectRatio="xMidYMid slice" className={styles.hero_decoration}>
        <path d="M-20,20 C260,40 480,180 700,324 S920,358 1000,358" fill="none" stroke="#8fd0e8" strokeWidth="1.2" opacity="0.5"></path>
        <path d="M-20,38 C260,54 480,192 700,332 S920,365 1000,365" fill="none" stroke="#7fa8c9" strokeWidth="1" opacity="0.38"></path>
        <path d="M-20,56 C260,68 480,204 700,340 S920,372 1000,372" fill="none" stroke="#3a75ad" strokeWidth="1" opacity="0.5"></path>
        <path d="M-20,500 C260,480 500,432 710,388 S920,380 1000,380" fill="none" stroke="#8fd0e8" strokeWidth="1.2" opacity="0.5"></path>
        <path d="M-20,485 C260,468 500,424 710,383 S920,374 1000,374" fill="none" stroke="#7fa8c9" strokeWidth="1" opacity="0.38"></path>
        <path d="M-20,470 C260,456 500,416 710,378 S920,368 1000,368" fill="none" stroke="#3a75ad" strokeWidth="1" opacity="0.5"></path>
      </svg>
      <div className={styles.hero_gradient}></div>
      <div className={clsx(styles.m_pad, styles.hero_inner)}>
        <div className={clsx(styles.m_hero, styles.hero_title)}><svg width="66" height="66" viewBox="0 0 120 120"><path d="M96,26 A46,46 0 1 0 96,94" fill="none" stroke="#8fd0e8" strokeWidth="5" strokeLinecap="round"></path><path d="M24,42 C48,44 66,56 104,60" fill="none" stroke="#8fd0e8" strokeWidth="3" strokeLinecap="round"></path><path d="M24,78 C48,76 66,64 104,60" fill="none" stroke="#3a75ad" strokeWidth="3" strokeLinecap="round"></path></svg>Confluence</div>
  ...
  return (
    <div className={clsx(styles.m_pad, styles.session_outer)}>
      ...
      <div className={clsx(styles.m_card, styles.m_stack, styles.session_card)}>
        ...
        <div className={clsx(styles.m_side, styles.session_bios_outer)}>
...
function ConfluenceApplication() {
  ...
  const [submitHref, setSubmitHref] = useState("mailto:ciroh@ua.edu?bcc=spaul5%40ua.edu&subject=Confluence%20%E2%80%94%20Operational%20Hydrology%20Advances%20Webinar%20Series%20%C2%B7%20PI%20Engagement%20Session");

  function handleNameChange(e) { setName(e.target.value); }
  function handleAffilChange(e) { setAffil(e.target.value); }
  function handleDateChange(e) { setDate(e.target.value); }
  function handleTopicChange(e) { setTopic(e.target.value); }

  useEffect(() => {
    const subject = encodeURIComponent('Confluence — Operational Hydrology Advances Webinar Series · PI Engagement Session');
    const body = encodeURIComponent('Name: ' + name + '\nAffiliation: ' + affil + '\nPreferred session: ' + (date||'—') + '\n\nProposed topic:\n' + topic);
    setSubmitHref('mailto:ciroh@ua.edu?bcc=spaul5%40ua.edu&subject='+subject+'&body='+body);
  }, [name, affil, date, topic]);

  return (
    <div id="apply" className={clsx(styles.m_pad, styles.application_outer)}>
      <div className={clsx(styles.m_card, styles.m_stack, styles.application_inner)}>
        <div style={{ flex: 1 }}>
        ...
        <div className={clsx(styles.m_full, styles.application_form_outer)}>
          <input id="apply-name" placeholder="Name" className={styles.application_field_text} onChange={handleNameChange} />
          <input id="apply-affiliation" placeholder="Affiliation" className={styles.application_field_text} onChange={handleAffilChange} />
          <div className={styles.application_column}>
            ...
            <div className={styles.application_row}>
              <label className={styles.application_field_choice}><input type="radio" name="apply-date" value="Oct 22, 2026" onChange={handleDateChange} />Oct 22, 2026</label>
              <label className={styles.application_field_choice}><input type="radio" name="apply-date" value="Mar 25, 2027" onChange={handleDateChange} />Mar 25, 2027</label>
              <label className={styles.application_field_choice}><input type="radio" name="apply-date" value="Either" onChange={handleDateChange} />Either</label>
            </div>
          </div>
          <textarea id="apply-topic" placeholder="Proposed topic" rows="3" className={styles.application_field_text} style={{ resize: "vertical" }} onChange={handleTopicChange}></textarea>
          <a href={submitHref} className={styles.button_l}>Submit nomination</a>
...
function ConfluenceRegistration() {
  return (
